...
export async function onRequest(context) {
  ...
  // if (config.homepage && url.pathname === '/proxy' && isProxyHost) {
...
class LinkRewriter {
  ...
  element(element) {
    ...
    try {
      ...
      const newURL = `https://${this.proxyDomain}/proxy?url=${absoluteURL.href}`
    ...
    if (httpEquiv && httpEquiv.toLowerCase() === 'refresh' && content) {
      ...
      if (parts.length === 2) {
        try {
          ...
          const newURL = `https://${this.proxyDomain}/proxy?url=${url.href}`
